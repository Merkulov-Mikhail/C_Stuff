...
int TestSquare (double a, double b, double c, double x1ref, double x2ref, int nRootsRef)
{
    ...
    if (nRoots == nRootsRef){
        ...
        if ( (doubleEquals (x1ref, x1) && doubleEquals (x2ref, x2)) || (doubleEquals (x1ref, x2) && doubleEquals (x2ref, x1))){
            return 0;
        }

        if (nRoots == 1 && (doubleEquals (x1ref, x1) || doubleEquals (x2ref, x1))){
            return 0;
        }
